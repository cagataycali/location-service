var ConversationalForm = function() {
  ...
  ConversationalFormDocs = function() {
    function t() {
      ...
      null !== document.getElementById("conversational-form-development") && this.el.classList.add("development"),
      this.h1writer = new H1Writer({el: document.getElementById("writer")}),
      "none" != window.getComputedStyle(document.getElementById("small-screen-menu")).getPropertyValue("display")
        ? this.introFlow1()
        : this.introFlow2()
    }
    ...
    t.prototype.toggleMenuState = function() {
      return this.el.classList.toggle("menu-toggle", !this.el.classList.contains("menu-toggle")) && this.el.classList.remove("cf-toggle"),
      !1
    },
    t.prototype.toggleConversation = function() {
      var t = this;
      return clearTimeout(this.introTimer),
      this.el.classList.contains("cf-toggle")
        ? this.el.classList.remove("cf-toggle")
        : (this.cf || (this.cf = new window.cf.ConversationalForm({
          ...
          flowStepCallback: function(e, s, i) {
            if (e.tag.domElement)
              if ("repeat" == e.tag.domElement.getAttribute("name"))
                location.reload();
              else if ("submit-form" == e.tag.domElement.getAttribute("name")) {
                var n = new XMLHttpRequest;
                n.addEventListener("load", function() {
                  t.cf.addRobotChatResponse("We received your submission 🙌"),
                  s()
                }),
                n.open("POST", document.getElementById("cf-form").getAttribute("action")),
                n.setRequestHeader("accept", "application/javascript"),
                n.setRequestHeader("Content-Type", "application/json"),
                n.send(JSON.stringify(t.cf.getFormData(!0)))
              } else
                s();
          else
              s()
          ...
          t.el.classList.remove("menu-toggle"),
          t.el.classList.add("cf-toggle")
        }, 10)),
      !1
    },
    t.start = function() {
  ...
  H1Writer = function() {
    ...
    t.prototype.nextStep = function() {
      0 == this.progressTarget && this.step++,
      this.str = this.strs[this.step],
      this.progressTarget = 0 == this.progressTarget
        ? 1
        : 0,
      this.render()
    },
    t.prototype.render = function() {
      var t = this;
      this.progress += (this.progressTarget - this.progress) * (0 == this.step
        ? .15
        : .09);
      var e = this.str.substr(0, Math.round(this.progress * this.str.length));
      this.el.innerHTML = e,
      Math.abs(this.progress - this.progressTarget) <= .01
        ? (cancelAnimationFrame(this.rAF), this.step < 1 && setTimeout(function() {
          t.nextStep()
        }, 500))
